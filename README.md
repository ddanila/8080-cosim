...
- The routed main-board artifact has 240 footprints and zero KiCad copper
  ...
  all twenty paired A-point landings remain absent from the PCB. The first two
  pairs (`А:19` and `А:20`) are registered in drawing-image space, with board
  coordinates and island assignment explicitly unset; the landing geometry is an adoption hold
