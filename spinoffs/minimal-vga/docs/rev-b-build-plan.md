...
| # | Question | Decision | Rationale / gate |
|---|---|---|---|
| D1.28 | How to close a *deterministic* single-net routing failure (cpu A8: 12/12 attempts)? | **Automated placement sweep first**: grid-search U1 (Z80) x-position (e.g. 25–45 mm in 2 mm steps) × rotation {90, 270} headlessly — regenerate, placement-DRC gate, 2 route attempts each, stop at the first DRC-0/0 full route. Only if the whole sweep fails: **one hand-authored track** emitted by the generator (documented, deterministic — not a GUI edit that regeneration would erase). | Stochastic retries are the wrong tool for a deterministic constraint; a sweep is desk-executable and stays within the everything-is-generated discipline. mem's U1-180° fix was exactly one sweep point found manually. **Resolved 2026-07-18 at x=41; R5.J2 clean-source refresh moved U1 to x=39 mm, rot=90 after x=41 repeatedly returned one ratline. The frozen x=39 source routes 0/0 on bounded attempt 1.** |
...
| # | Question | Decision | Rationale / gate |
|---|---|---|---|
| D1.35 | The backplane — the power-entry board — has **zero capacitors**. Ship as is? | **No — add input power conditioning**: one bulk electrolytic (47 µF/16 V radial) plus one 100 nF ceramic. **R5.I7 amendment:** five-board power enters through exact barrel `J_PWR`, then MF-R300, with an SB560 reverse-polarity crowbar. **R5.J1 amendment:** omit the optional USB power branch. | The protected barrel path is qualified for the 1.655 A machine; `five-board-physical.json` and its negative-tested checker supersede the historical direct-header and USB-service wording. |
...
| D1.38 | What supply and distribution corner releases the VGA-populated machine? | **Mean Well GST25A05-P1J, individually receipt-tested at 1.655 A for >=4.90 V average and <=80 mVpp; Video in slot 5, slot 4 empty.** Route VCC/GND buses at 0.80 mm and raw barrel-to-fuse copper at 2.00 mm on 1 oz. | R5.I7 actual-copper solve including maximum initial fuse/jack/contact losses yields 4.574 V minimum trough, 74 mV above the 4.50 V floor. The datasheet-only -5% adapter corner yields 4.424 V, so receipt testing is a real gate, not optional prose. |
