...
| Slot count / pitch | **6 slots @ 16 mm** on a 100×120 mm backplane | Mating-contract result (D1.31): preserves six slots and the power tail; the one-off backplane crosses the cheap-tier height while cards remain ≤100×100 |
| Termination / buffering | **No bus termination; CPU unbuffered in B1** (D1.21) | Mainline RC2014 runs unbuffered at 7.37 MHz; VJUGA targets ~2–4 MHz on six short slots. Bench timing decides whether a later buffered CPU revision is useful |
...
1. **Backplane** — passive, 100×120, 2-layer, 6 slots; + USB-C 5V in, reset
...
ROM in. The three B1 cards remain ≤100×100; the deliberately larger 100×120 backplane is
the D1.31/D1.35 exception. Exit: monitor prompt + RAM test PASS over the backplane FTDI
header.
...
| S10 | Expansion beyond 6 slots? | **Future second-backplane/daisy-chain option; not fitted in the B1 first article.** | Six slots cover B1–B4; do not add unverified expansion copper before bench. |
...
| D1.30 | Where to place base vs ext connectors on the backplane? | **Historical two-bank layout, rejected by the D1.31 mating audit.** Production uses co-located base/ext socket pairs from `mating.json`; their columns interleave, and the power tail occupies the lower strip of the 100×120 board. | Routing success does not matter if the cards cannot mate; D1.31 made geometry executable and caught this layout. |

## Stage D design decisions (from Stage C exit review, 2026-07-18)

| # | Question | Decision | Rationale / gate |
|---|---|---|---|
| D1.31 | How do cards physically mate with the backplane — and what enforces it? | **A machine-checked mechanical mating contract**, like the bus contract but for geometry. Current `mating.json`: card base row x=50 at **4.0 mm** above the bottom edge, ext row x=14.45 at **9.0 mm** (5 mm separation); backplane slot k carries the same pair, **16 mm pitch**, six slots, 100×120 outline with the tail starting at y=98. The generator and `check_revb_mating.py` consume these exact constants. | The Stage-C audit caught mutually inconsistent card rows and a non-mating two-bank backplane. The executable contract prevents that class of silent mechanical failure. |
...
| D1.36 | Six backplane footprint classes were initially name-matched only. What closes them? | **Pinned parts + physical guards.** Production uses fully-THT GCT USB4085, APEM MJTP1243, DS1813-5 (pad 1=/RESET, 2=VCC, 3=GND), Bourns MF-R110, and the recorded standard THT parts. `PKG_PHYS` checks through-hole count/minimum drill/pitch; `PKG_WIDTH` checks DIP row spacing; the negative self-test proves rejection of the wrong SMD USB-C and 300-mil DIP-28. | A footprint is only verified relative to a physical part; machine guards keep that result from regressing. |
