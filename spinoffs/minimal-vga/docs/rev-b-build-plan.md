...
| D2.9 | **[OPEN]** The integrated boot (ekta37 through the chip-level TTL card, /WAIT wired to the real T80) LOSES framebuffer writes — why, and the fix? | **Finding (TI.3):** the unit /WAIT sweep passed with a stub CPU, but wiring the card's open-drain /WAIT into the actual T80 and booting ekta37, framebuffer writes during the active region **vanish** (the dump shows 0 where cosim has pixels) instead of being deferred to blanking — the CPU is not being held the way the stub was. Two candidate causes to chase: (a) the T80's /WAIT sampling vs. the card's write-grant edge don't align (the write M-cycle completes before the grant fires); (b) even synchronized, blocking the CPU for 73% of every frame is a poor design — the **right fix is likely cycle-stealing** (the 25 MHz dot clock has ~12 idle dots between CPU accesses and only fetches 1 byte/16 dots, so a CPU access can be interleaved into the scanout stream with ≤1-dot wait, and writes ALWAYS land), replacing D2.5's block-and-wait. Added an `active`→CPU-domain 2-FF synchronizer (real CDC bug, kept) but that alone didn't fix it. **The integrated boot gate is committed but GATED OFF (`REVB_TTL_BOOT=1`) until this is resolved; the behavioural partition boot stays green.** | An integrated gate earns its keep by finding exactly this: a unit-passing design that breaks against the real CPU. Netlist + LVS are done and verified; the contention scheme needs a focused rework (cycle-steal) as its own step. |
| D2.5 | CPU/scanout contention strategy? | **Scanout has absolute priority; CPU waits.** *(Superseded direction — D2.9: the block-and-wait scheme loses writes against the real T80; likely moving to cycle-stealing.)* The GAL asserts open-drain `WAIT_N` (the D1.4 ext line, pulled up on the backplane) for CPU framebuffer accesses that collide with active-region fetches; accesses land in blanking otherwise. Gate: the TI.2 **/WAIT phase sweep** (a CPU access launched at every dot-clock phase) must show zero lost/corrupted accesses and bounded wait. Mode overlays (MODE0/1) decode per the facts overlay table. | A fixed-priority design is provable by sweep; arbitration cleverness is not. This is exactly what the 10-pin extension exists for. |
