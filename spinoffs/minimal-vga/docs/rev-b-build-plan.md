...
| # | Question | Decision | Rationale / gate |
|---|---|---|---|
| D1.35 | The backplane — the power-entry board — has **zero capacitors**. Ship as is? | **No — add input power conditioning**: one bulk electrolytic (47 µF/16 V radial) plus one 100 nF ceramic and an MF-R110 in the USB branch. **R5.V6 amendment:** USB-C is service-only; normal five-board power now enters through exact barrel `J_PWR`, then MF-R250, with an SB560 reverse-polarity crowbar. | The original USB protection remains useful, but its 1.1 A hold rating cannot power the 1.351 A machine. The normal path is now fused too; `five-board-physical.json` and its negative-tested checker supersede the historical direct-header wording. |
...
| D1.37 | Can the backplane return to the 100×100 cheap tier without compromising the planned system? | **Yes: use five slots at the proven 16 mm pitch.** Slots 1–5 support CPU, Memory, I/O, Video and FDC; the removed sixth slot was only spare expansion. Its space becomes a clear top strip for top-side USB-C, reset, power and serial hardware, avoiding the unresolved seated-card collision risk of a six-slot inter-slot layout. The generated board is 100×100, placement-clean, routes 0/0 on attempt 1, passes the mating/footprint/tier gates, and has a validated fabrication package. | Preserves every planned tier and mechanical margin while crossing back below the vendor price cliff. Expansion beyond five simultaneous cards now means a later second/revised backplane. |
| D1.38 | What supply and distribution corner releases the VGA-populated machine? | **Mean Well GST25A05-P1J, individually receipt-tested at 1.351 A for >=4.90 V average and <=80 mVpp; Video in slot 5, slot 4 empty.** Route VCC/GND buses at 0.80 mm and raw barrel-to-fuse copper at 2.00 mm on 1 oz. | Actual-copper nodal solve including maximum initial fuse/jack/contact losses yields 4.624 V minimum trough, 124 mV above the 4.50 V floor. The datasheet-only -5% adapter corner yields 4.474 V, so receipt testing is a real gate, not optional prose. |
