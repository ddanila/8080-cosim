...
Status: **R5.S3 electrical boundary and C10 protocol path implemented and
desk-verified 2026-08-27**.
...
- Separate 10 kΩ pull-ups hold `CON_TX_SRC` and bus `RX` high when either
  `JP_S5` shunt is removed. Isolation therefore has a defined idle state on both
  sides instead of leaving an HCT or 8251 input floating.
...
## Automated protocol and loopback evidence

`revb_serial_console_check.sh` drives the repository's real `usart_8251` model
through the protected connector boundary. It proves active-high 8251 reset,
idle-high isolation with both shunts open, exact 19,200 and 9,600 x16 clocks,
bidirectional 8N1 bytes, and a temporary `J_TTL` 2–3 external loopback. The test
also exchanges one exact ABI 1.4 C10 PROBE/DATA transaction generated by the
shared JukuRavi protocol implementation. A deliberate isolation-bypass run must
