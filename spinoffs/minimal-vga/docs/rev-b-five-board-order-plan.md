...
| **R5.S1** | Audit the complete 8251-to-backplane serial net path. Correct stale `JP_S5` wording so it describes console isolation/crossover, not a nonexistent Serial card. Freeze a board-relative header pinout (`5V`, board TX, board RX, GND) and conspicuous `TTL ONLY — NOT RS-232` silk. | R5.0 | Schematic/netlist/board contract and silk checker agree; TX/RX direction has an automated continuity check. |
