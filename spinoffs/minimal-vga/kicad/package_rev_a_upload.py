...
def write_readme(out_dir):
    ...
    lines = [
        ...
        "Upload `vjuga-rev-a-jlcpcb-bom.csv` and `vjuga-rev-a-jlcpcb-cpl.csv`",
        "as the factory assembly BOM/CPL pair.",
        "",
        ...
        "",
        "Review the manual assembly list, post-assembly insertion list, assembly",
        "notes, root `order-readiness.md`, and SHA256 checksums before order upload.",
        "",
    ]
    path.write_text("\n".join(lines))
    return path
...
def write_manifest(out_dir, files, sha_path):
    ...
    lines.extend(
        [
            ...
            "- Upload the generated JLCPCB BOM/CPL CSV pair for assembly.",
            "- Keep manual and post-assembly insertion files with the order notes.",
            "- Re-check JLCPCB stock and run independent Gerber review before order.",
            "",
        ]
    )
    path.write_text("\n".join(lines))
