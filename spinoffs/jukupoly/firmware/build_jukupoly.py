...
def compile_song(song: dict) -> tuple[str, dict]:
    ...
    for row_index, row in enumerate(rows):
        ...
        for channel, flag in TONE_FLAGS.items():
            ...
            if "opl_tremolo_depth" in event and not enhanced_envelopes:
                raise SongError(
            ...
            if enhanced_envelopes:
                ...
                depth = event.get("opl_tremolo_depth", 0)
                if isinstance(depth, bool):
                    raise SongError(
                        f"row {row_index} {channel} opl_tremolo_depth must "
                        f"be 0..3, got {depth!r}"
                    )
                depth = check_range(
                    f"row {row_index} {channel} opl_tremolo_depth",
                    depth, 0, 3,
        ...
        output.append(
            f"        db      {asm_hex(frames, 2)},{asm_hex(flags, 2)}"
            f"        ; row {row_index}"
        )
        output.extend(payload)

    output.extend([
        f"        db      00h,{asm_hex(END_FLAG, 2)}        ; end",
        "",
        "jukupoly_silence:",
...
def assemble_song_file(generated: str, metadata: dict) -> bytes:
    ...
    with tempfile.TemporaryDirectory(prefix="jukupoly-song.") as name:
        ...
        source.write_text(
            ...
            "        dw      jukupoly_silence\n"
            "        dw      0000h\n"
            f"        include \"{DEFAULT_GENERATED.name}\"\n"
            "jukupoly_song_image_end:\n"
            "        end     jukupoly_song_image\n"
        )
        command = [
            str(executable()), "--nmnv", "--zmac", "-8",
            f"-I{directory}", "-o", str(image), str(source),
        ...
        if len(payload) >= 0x8000:
            raise SongError(
                f"JPS image is {len(payload)} bytes; library limit is 32767"
            )
        return payload
...
def main() -> int:
    ...
    parser.add_argument("--song-output", type=Path,
                        help="also write a fixed-address JPS disk-song image")
    ...
    if args.check:
        ...
        if (args.song_output is not None and
                (not args.song_output.exists() or
                 args.song_output.read_bytes() != song_image)):
            raise SystemExit(f"{args.song_output} is missing or stale")
        action = "checked"
    else:
        args.output.write_bytes(image)
        if args.song_output is not None:
            args.song_output.write_bytes(song_image)
        action = "wrote"
    print(
        ...
        + (f" jps={len(song_image)}" if song_image is not None else "")
    )
    return 0
