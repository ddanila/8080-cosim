#!/usr/bin/env python3
"""Build Jukuravi cumulative audible PIT-checkpoint diagnostic."""
...
OUTPUT = HERE / "diag-d0-pit-debug.bin"
README = HERE / "README.md"
IDENTITY = b"JUKURAVI-D0-PIT-DEBUG-1\0"
ROM_VERSION = 11
...
def build() -> tuple[bytes, dict[str, int | list[int] | bytes]]:
    ...
    if stored != computed or stored != metadata["rom_checksum"]:
        raise ValueError("PIT-debug block-1 checksum mismatch")
    if metadata["pit_debug_checkpoints"] != list(range(1, 13)):
        raise ValueError("PIT-debug checkpoint numbering is incomplete")
    if metadata["local_timeout_offsets"] or metadata["pit_debug"] != 1:
        raise ValueError("PIT-debug image retained a USART dependency")
...
def main() -> int:
    ...
    if args.check:
        if not OUTPUT.exists() or OUTPUT.read_bytes() != image:
            raise SystemExit("diag-d0-pit-debug.bin is missing or stale")
        if not README.exists() or digest not in README.read_text():
            raise SystemExit("firmware README does not pin the PIT-debug SHA256")
        action = "checked"
    else:
        OUTPUT.write_bytes(image)
        action = "wrote"
    print(
        f"JUKURAVI-D0-PIT-DEBUG-BUILD: {action} {OUTPUT.name} "
