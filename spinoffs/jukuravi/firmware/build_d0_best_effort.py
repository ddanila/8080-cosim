#!/usr/bin/env python3
"""Build a RAM-independent, dependency-aware Jukuravi diagnostic ROM."""
...
from build_d0_ram_fallback import (
    FALLBACK_WINDOWS, PIC_COMMAND_PORT, PIC_DATA_PORT, PIC_ICW1, PIC_ICW2,
...
OUTPUT = HERE / "diag-d0-best-effort.bin"
README = HERE / "README.md"
IDENTITY_OFFSET = 0x1F00
IDENTITY = b"JUKURAVI-D0-BEST-EFFORT-1\0"
ROM_VERSION = 14
...
def build() -> tuple[bytes, dict[str, int | list[int] | bytes]]:
    ...
    emit_status_wait(asm, stem="best_banner_empty", mask=0x04,
                     failure_label="uart_dead")
    emit_ack_rx(asm, len(placeholder_ack), failure_label="uart_dead")
    ...
    # Three long high pulses and silence mean UART report + usable RAM window.
    asm.emit(0x16, 0x03)
    emit_slow_grouped_pulse_loop(
        asm, stem="best_complete", divisor=WINDOWS_FOUND_DIVISOR,
        done_label="best_complete_halt",
    )
    asm.label("best_complete_halt")
    success_halt = asm.pc
    asm.emit(0x76)
...
def main() -> int:
    ...
    if args.check:
        if not OUTPUT.exists() or OUTPUT.read_bytes() != image:
            raise SystemExit("diag-d0-best-effort.bin is missing or stale")
        if not README.exists() or digest not in README.read_text():
            raise SystemExit("firmware README does not pin best-effort SHA256")
        action = "checked"
    else:
        OUTPUT.write_bytes(image); action = "wrote"
    print(f"JUKURAVI-D0-BEST-EFFORT-BUILD: {action} {OUTPUT.name} bytes={len(image)} "
