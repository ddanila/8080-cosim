...
OUTPUT = HERE / "diag-d0-robust.bin"
README = HERE / "README.md"
IDENTITY_OFFSET = 0x1F00
IDENTITY = b"JUKURAVI-D0-ROBUST-SERIAL-1\0"
ROM_VERSION = 15
...
def emit_robust_ack_rx(asm: Assembler, count: int) -> None:
    """Find one exact ACK amid bounded garbage and repeated ACK frames.

    E contains the peripheral fault bitmap on entry and exit.  The first-byte
    wait retains the long hardware timeout.  Once a stream begins, a 256-poll
    inter-byte window is comfortably longer than one 9600-baud character.
    """
    label_address = lambda opcode, label: (
        asm.emit(opcode, 0x00, 0x00), asm.fixups.append((asm.pc - 2, label))
    )
    label_address(0x21, "ack_expected")  # LXI H,expected
    asm.emit(0x16, ACK_SCAN_BUDGET)       # MVI D,scan budget
    emit_status_wait(asm, stem="robust_ack_first", mask=0x02,
                     failure_label="uart_dead")
    asm.emit(0x43)                         # MOV B,E: preserve fault bitmap
    asm.emit(0x1E, count)                  # MVI E,remaining frame bytes
    asm.emit(0xDB, 0x08)                   # IN first data byte
    asm.jump(0xC3, "robust_ack_compare")

    asm.label("robust_ack_wait")
    asm.emit(0x0E, 0x00)                   # MVI C,0 => 256 polls
    asm.label("robust_ack_poll")
    asm.emit(0xDB, 0x09, 0xE6, 0x02)
    asm.jump(0xC2, "robust_ack_seen")
    asm.emit(0x0D)                         # DCR C
    asm.jump(0xC2, "robust_ack_poll")
    asm.jump(0xC3, "uart_dead")
    asm.label("robust_ack_seen")
    asm.emit(0xDB, 0x08)                   # IN data

    asm.label("robust_ack_compare")
    asm.emit(0xBE)                         # CMP M
    asm.jump(0xCA, "robust_ack_match")
    asm.emit(0x15)                         # DCR D: bounded garbage
    asm.jump(0xCA, "uart_dead")
    # Resynchronize.  A mismatching A5 is itself the first byte of a new frame.
    label_address(0x21, "ack_expected")
    asm.emit(0x1E, count, 0xFE, 0xA5)      # MVI E,count / CPI A5
    asm.jump(0xC2, "robust_ack_wait")
    asm.emit(0x23, 0x1D)                   # INX H / DCR E
    asm.jump(0xC3, "robust_ack_wait")

    asm.label("robust_ack_match")
    asm.emit(0x23, 0x1D)                   # INX H / DCR E
    asm.jump(0xC2, "robust_ack_wait")
...
def build() -> tuple[bytes, dict[str, int | list[int] | bytes]]:
    ...
    emit_status_wait(asm, stem="best_banner_empty", mask=0x04,
                     failure_label="uart_dead")
    emit_robust_ack_rx(asm, len(placeholder_ack))
...
def main() -> int:
    ...
    if args.check:
        if not OUTPUT.exists() or OUTPUT.read_bytes() != image:
            raise SystemExit("diag-d0-robust.bin is missing or stale")
        if not README.exists() or digest not in README.read_text():
            raise SystemExit("firmware README does not pin robust-full SHA256")
        action = "checked"
    else:
        OUTPUT.write_bytes(image); action = "wrote"
    print(f"JUKURAVI-D0-ROBUST-FULL-BUILD: {action} {OUTPUT.name} bytes={len(image)} "
