#!/usr/bin/env python3
"""Build T29 with post-diagnostic markers and bounded serial recovery."""
...
from build_d3_loader_v3 import emit_loader as emit_loader_v3


OUTPUT = base.HERE / "diag-d0-host-recover.bin"
DOS_OUTPUT = base.HERE / "dos" / "T29HOST.BIN"
README = base.HERE / "README.md"
ROM_VERSION = 0x18
IDENTITY = b"JUKURAVI-D0-HOST-RECOVER-2400-1\0"
...
def build():
    ...
    try:
        ...
        base.LOADER_EMITTER = emit_loader_v3
        ...
        base.REQUIRE_FINAL_TX_EMPTY = True
...
def main() -> int:
    ...
    if args.check:
        ...
        if not README.exists() or digest not in README.read_text():
            raise SystemExit("firmware README does not pin the T29 image SHA256")
        action = "checked"
    else:
        ...
        action = "wrote"
    print(
        f"JUKURAVI-D0-HOST-RECOVER-BUILD: {action} {OUTPUT.name} "
