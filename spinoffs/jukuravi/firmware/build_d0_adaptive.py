...
OUTPUT = HERE / "diag-d0-adaptive.bin"
README = HERE / "README.md"
IDENTITY_OFFSET = 0x1F00
IDENTITY = b"JUKURAVI-D0-ADAPTIVE-SERIAL-1\0"
ROM_VERSION = 17
...
def build() -> tuple[bytes, dict[str, int | list[int] | bytes]]:
    ...
    loader_metadata = emit_loader(asm, encoded_input=True)
...
def main() -> int:
    ...
    if args.check:
        if not OUTPUT.exists() or OUTPUT.read_bytes() != image:
            raise SystemExit("diag-d0-adaptive.bin is missing or stale")
        if not README.exists() or digest not in README.read_text():
            raise SystemExit("firmware README does not pin adaptive SHA256")
        action = "checked"
    else:
        OUTPUT.write_bytes(image); action = "wrote"
    print(f"JUKURAVI-D0-ADAPTIVE-FULL-BUILD: {action} {OUTPUT.name} bytes={len(image)} "
