#!/usr/bin/env python3
"""Build Jukuravi cumulative no-serial audible diagnostic."""
...
def build() -> tuple[bytes, dict[str, int | list[int] | bytes]]:
    ...
    if stored != computed or stored != metadata["rom_checksum"]:
        raise ValueError("no-serial block-1 checksum mismatch")
    if metadata["no_serial_fallback"] != 1 or metadata["local_timeout_offsets"]:
        raise ValueError("no-serial image retained a USART dependency")
...
def main() -> int:
    ...
    if args.check:
        ...
        action = "wrote"
    print(
        f"JUKURAVI-D0-NOSERIAL-BUILD: {action} {OUTPUT.name} "
