...
from build_d2_loader import emit_loader as emit_loader_v1
...
OUTPUT = HERE / "diag-d0-resilient.bin"
README = HERE / "README.md"
IDENTITY_OFFSET = 0x1F00
IDENTITY = b"JUKURAVI-D0-RESILIENT-2400-1\0"
ROM_VERSION = 19
SOLICITED_INPUT = False
FILTER_INVALID_SYMBOLS = False
...
REQUIRE_BANNER_TX_EMPTY = True
REQUIRE_FINAL_TX_EMPTY = True
DIRECT_LOADER_HANDOFF = False
ROBUST_BAUD_DIVISOR = 32  # nominal 1.23 MHz / 32 / 16 ~= 2400 baud
...
SAFE_CHALLENGE = bytes((0x55, 0xAA, 0x55, 0xAA, 0x55, 0xAA))
...
def emit_adaptive_handshake(asm: Assembler) -> None:
    """Challenge/echo only hardware-proven symbols; report mismatched bytes."""
    ...
    label_address(0x21, "safe_challenge")
    ...
    asm.emit(0xDB, 0x08, 0x57, 0xBE)       # IN data / MOV D,A / CMP M
    asm.jump(0xCA, "stopwait_match")
    # Outbound telemetry is reliable on CS00015. F0,expected,actual records
    # the exact receive corruption without changing the challenge cursor.
    for suffix, source in (("mark", 0xF0), ("expected", "m"), ("actual", "d")):
        asm.label(f"stopwait_telemetry_{suffix}_wait")
        asm.emit(0xDB, 0x09, 0xE6, 0x01)
        asm.jump(0xCA, f"stopwait_telemetry_{suffix}_wait")
        if source == "m":
            asm.emit(0x7E)
        elif source == "d":
            asm.emit(0x7A)
    ...
    asm.emit(0xFE, (0x0812 + len(SAFE_CHALLENGE)) & 0xFF)
...
def emit_loader_workspace_test(
    asm: Assembler, *, base: int = FRAMEBUFFER_BASE,
    size: int = FRAMEBUFFER_BYTES,
) -> None:
    """Prove the selected loader buffer/stack workspace without a stack."""
...
def emit_loader_guard(
    asm: Assembler, *, progress_marker: int | None = None
) -> tuple[int, int]:
    ...
    if progress_marker is not None:
        tx_byte(
            asm,
            "postdiag_loader_guard_progress",
            progress_marker,
...
def build() -> tuple[bytes, dict[str, int | list[int] | bytes]]:
    ...
    emit_local_usart_test(
        asm, failure_label="uart_dead", baud_divisor=ROBUST_BAUD_DIVISOR
    )
    ...
    emit_table_tx(asm, "banner", len(placeholder_banner), stem="best_banner",
                  failure_label="uart_dead")
    if REQUIRE_BANNER_TX_EMPTY:
        emit_status_wait(asm, stem="best_banner_empty", mask=0x04,
                         failure_label="uart_dead")
    emit_adaptive_handshake(asm)
    ...
    # A compact recovery build can enter a fully sub-1000h loader directly on
    # boards where execution from D15's A12=1 half is not trustworthy.
    asm.jump(0xC3, "loader_entry" if DIRECT_LOADER_HANDOFF else "robust_postdiag")
    success_halt = -1
    ...
    asm.label("safe_challenge")
    asm.emit(*SAFE_CHALLENGE)
    ...
    loader_metadata = LOADER_EMITTER(
        ...
        verify_buffer_stores=VERIFY_BUFFER_STORES,
    )
    asm.label("loader_extension_end")
    asm.label("robust_postdiag")
    if POSTDIAG_PROGRESS_MARKERS is not None:
        tx_byte(
            asm,
            "postdiag_enter_progress",
            POSTDIAG_PROGRESS_MARKERS[0],
            failure_label="uart_dead",
        )
    emit_loader_workspace_test(
        asm, base=LOADER_WORKSPACE_BASE, size=LOADER_WORKSPACE_BYTES,
    )
    if POSTDIAG_PROGRESS_MARKERS is not None:
        tx_byte(
            asm,
            "postdiag_workspace_progress",
            POSTDIAG_PROGRESS_MARKERS[1],
            failure_label="uart_dead",
        )
    loader_length_offset, loader_checksum_offset = emit_loader_guard(
        asm,
        progress_marker=(
            None
            if POSTDIAG_PROGRESS_MARKERS is None
...
def main() -> int:
    ...
    if args.check:
        if not OUTPUT.exists() or OUTPUT.read_bytes() != image:
            raise SystemExit("diag-d0-resilient.bin is missing or stale")
        if not README.exists() or digest not in README.read_text():
            raise SystemExit("firmware README does not pin resilient SHA256")
        action = "checked"
    else:
        OUTPUT.write_bytes(image); action = "wrote"
    print(f"JUKURAVI-D0-RESILIENT-FULL-BUILD: {action} {OUTPUT.name} bytes={len(image)} "
