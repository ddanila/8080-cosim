...
def main():
    ...
    # The live EKDOS command probe loads a BASIC-shaped payload from JUKPROG2
    # that is not the same byte stream as the conservative directory extraction.
    # Preserve it explicitly until the disk allocation mapping is fully resolved.
    jukprog2_live_offset = 0x2DE00
    jukprog2_live_len = jukprog2_jbasic_entry["records"] * CPM_RECORD_SIZE
    jukprog2_live_candidate = jukprog2[jukprog2_live_offset : jukprog2_live_offset + jukprog2_live_len]
    jukprog2_live_candidate_path = write_artifact("JUKPROG2_JBASIC_live_candidate.COM", jukprog2_live_candidate)

    artifacts = [
        (jukprog2_jbasic_path, jukprog2_jbasic),
        (jukprog2_live_candidate_path, jukprog2_live_candidate),
    ...
    jukprog2_live_strings = string_offsets(jukprog2_live_candidate, ["BASIC", "READY", "ERROR", "COPYRIGHT"])
    rows.append(
        [
            "JUKPROG2.CPM",
            f"raw 0x{jukprog2_live_offset:05X}",
            "JBASIC.COM live-load candidate",
            len(jukprog2_live_candidate),
            "-",
            jukprog2_live_candidate[:8].hex(" "),
            ", ".join(f"`{key}`@0x{value:04X}" for key, value in jukprog2_live_strings.items()) or "-",
            sha256(jukprog2_live_candidate),
        ]
    )
    ...
    lines = [
        ...
        table_row([rel(jukprog2_live_candidate_path), "`JUKPROG2.CPM` raw live-load candidate at `0x2DE00`", sha256(jukprog2_live_candidate)]),
