...
def main() -> int:
    ...
    if not live_prune_path.exists():
        ...
        if (
            ...
            or final.get("uncapped_unconnected") != 1
            or final.get("routed_nets") != 410
            ...
            or probe.get("accepted_routes_by_uncapped_guard") != 676
            or probe.get("uncapped_unconnected_before") - probe.get("uncapped_unconnected_after") != 676
            or [item.get("net") for item in live_prune.get("config", {}).get("ripup_transactions", [])] != ["DB4", "VA14", "CS_D26", "PROM_EN", "DB5", "IOWR_RAW_N", "INT7_RAW", "D94_D0_BOUNDARY", "MEMW"]
            ...
            or probe.get("rule_accurate_multilayer_max_candidate_distance_mm") != 252.637
            or probe.get("rule_accurate_multilayer_all_distance_candidates_exhausted") is not True
            or probe.get("fine_multilayer_clearance_mm") != 0.205
            or probe.get("fine_multilayer_grid_step_mm") != 0.125
            ...
            or probe.get("multilayer_all_distance_candidates_exhausted") is not True
    ...
    if not current7_path.exists():
        failures.append("current seven-open residual-topology evidence is missing")
    else:
        current7 = json.loads(current7_path.read_text(encoding="utf-8"))
        entries = current7.get("residuals", [])
        transaction = current7.get("transaction", {})
        if (
            current7.get("schema_version") != 1
            or current7.get("board_sha256")
            != "16b8c6ed89eff11910e186414eccb95845fcf2376ced10e0cc9ba1a6c2a79c89"
            or current7.get("board_size") != 10270058
            or current7.get("uncapped_unconnected") != 7
            or current7.get("electrical_blockers") != 0
            or (current7.get("track_dangling"), current7.get("via_dangling"))
            != (0, 0)
            or current7.get("promoted_closures") != 670
            or transaction.get("net") != "CS_D26"
            or transaction.get("additional_closed_net") != "GND"
            or transaction.get("removed_source_items") != 0
            or len(entries) != 7
            or {entry.get("net") for entry in entries}
            != {"ROM_CS_EXP18", "IOWR_RAW_N", "D3_O6_D6_A5", "INT7_RAW", "CS_D57", "CS_D55", "D94_D0_BOUNDARY"}
        ):
            failures.append("current seven-open residual-topology summary is malformed")
        for relative, expected in current7.get("tool_sha256", {}).items():
            tool_path = ROOT / relative
            if not tool_path.is_file() or sha256(tool_path) != expected:
    ...
    if not current5_path.exists():
        failures.append("current five-open residual-topology evidence is missing")
    else:
        current5 = json.loads(current5_path.read_text(encoding="utf-8"))
        entries = current5.get("residuals", [])
        transactions = current5.get("transactions", [])
        if (
            current5.get("schema_version") != 1
            or current5.get("board_sha256")
            != "d026d4c562c42fcd19d368cd197ec857fa95af08c1e7048300383780197b4205"
            or current5.get("board_size") != 10414896
            or current5.get("uncapped_unconnected") != 5
            or current5.get("electrical_blockers") != 0
            or (current5.get("track_dangling"), current5.get("via_dangling")) != (0, 0)
            or current5.get("promoted_closures") != 672
            or [item.get("net") for item in transactions] != ["CS_D55", "DB5"]
            or any(item.get("removed_source_items") != 0 for item in transactions)
            or len(entries) != 5
            or {entry.get("net") for entry in entries}
            != {"ROM_CS_EXP18", "IOWR_RAW_N", "D3_O6_D6_A5", "INT7_RAW", "D94_D0_BOUNDARY"}
        ):
            failures.append("current five-open residual-topology summary is malformed")
        for relative, expected in current5.get("tool_sha256", {}).items():
            tool_path = ROOT / relative
            if not tool_path.is_file() or sha256(tool_path) != expected:
    ...
    if not current4_path.exists():
        failures.append("current four-open residual-topology evidence is missing")
    else:
        current4 = json.loads(current4_path.read_text(encoding="utf-8"))
        entries = current4.get("residuals", [])
        transaction = current4.get("transaction", {})
        if (
            current4.get("schema_version") != 1
            or current4.get("board_sha256")
            != "0bda1d229e84fd1b55d2adef44d16b23a3ac06234853164cbe9b6a188c3c0227"
            or current4.get("board_size") != 10449077
            or current4.get("uncapped_unconnected") != 4
            or current4.get("electrical_blockers") != 0
            or (current4.get("track_dangling"), current4.get("via_dangling")) != (0, 0)
            or current4.get("promoted_closures") != 673
            or transaction.get("net") != "IOWR_RAW_N"
            or transaction.get("removed_source_items") != 0
            or len(entries) != 4
            or {entry.get("net") for entry in entries}
            != {"ROM_CS_EXP18", "D3_O6_D6_A5", "INT7_RAW", "D94_D0_BOUNDARY"}
        ):
            failures.append("current four-open residual-topology summary is malformed")
        for relative, expected in current4.get("tool_sha256", {}).items():
            tool_path = ROOT / relative
            if not tool_path.is_file() or sha256(tool_path) != expected:
    ...
    if not current3_path.exists():
        failures.append("current three-open residual-topology evidence is missing")
    else:
        current3 = json.loads(current3_path.read_text(encoding="utf-8"))
        entries = current3.get("residuals", [])
        transaction = current3.get("transaction", {})
        if (
            current3.get("schema_version") != 1
            or current3.get("board_sha256")
            != "30029891e5fb5dbc654ee06a01b43dba0c12fa87dff61cd7c75d9a19e1544478"
            or current3.get("board_size") != 10518817
            or current3.get("uncapped_unconnected") != 3
            or current3.get("electrical_blockers") != 0
            or (current3.get("track_dangling"), current3.get("via_dangling")) != (0, 0)
            or current3.get("promoted_closures") != 674
            or transaction.get("net") != "INT7_RAW"
            or transaction.get("removed_source_items") != 0
            or len(entries) != 3
            or {entry.get("net") for entry in entries}
            != {"ROM_CS_EXP18", "D3_O6_D6_A5", "D94_D0_BOUNDARY"}
        ):
            failures.append("current three-open residual-topology summary is malformed")
        for relative, expected in current3.get("tool_sha256", {}).items():
            tool_path = ROOT / relative
            if not tool_path.is_file() or sha256(tool_path) != expected:
    ...
    if not current2_path.exists():
        failures.append("current two-open residual-topology evidence is missing")
    else:
        current2 = json.loads(current2_path.read_text(encoding="utf-8"))
        entries = current2.get("residuals", [])
        transaction = current2.get("transaction", {})
        if (
            current2.get("schema_version") != 1
            or current2.get("board_sha256")
            != "a611fb02103d789dc14e004ebd454e03b9dc43e0eb24139f6c88f00d66e2866b"
            or current2.get("board_size") != 10570467
            or current2.get("uncapped_unconnected") != 2
            or current2.get("electrical_blockers") != 0
            or (current2.get("track_dangling"), current2.get("via_dangling")) != (0, 0)
            or current2.get("promoted_closures") != 675
            or transaction.get("net") != "D94_D0_BOUNDARY"
            or transaction.get("removed_source_items") != 0
            or len(entries) != 2
            or {entry.get("net") for entry in entries}
            != {"ROM_CS_EXP18", "D3_O6_D6_A5"}
        ):
            failures.append("current two-open residual-topology summary is malformed")
        for relative, expected in current2.get("tool_sha256", {}).items():
            tool_path = ROOT / relative
            if not tool_path.is_file() or sha256(tool_path) != expected:
    ...
    if not current1_path.exists():
        failures.append("current one-open residual-topology evidence is missing")
    else:
        current1 = json.loads(current1_path.read_text(encoding="utf-8"))
        entries = current1.get("residuals", [])
        transaction = current1.get("transaction", {})
        if (
            current1.get("schema_version") != 1
            or current1.get("board_sha256") != "c62db440e2c8f7cd2e5617571a94fd453292dfebae314caa4671462521742b26"
            or current1.get("board_size") != 10641592
            or current1.get("uncapped_unconnected") != 1
            or current1.get("electrical_blockers") != 0
            or (current1.get("track_dangling"), current1.get("via_dangling")) != (0, 0)
            or current1.get("promoted_closures") != 676
            or transaction.get("net") != "MEMW"
            or transaction.get("removed_source_items") != 0
            or len(entries) != 1
            or entries[0].get("net") != "ROM_CS_EXP18"
        ):
            failures.append("current one-open residual-topology summary is malformed")
        for relative, expected in current1.get("tool_sha256", {}).items():
            tool_path = ROOT / relative
            if not tool_path.is_file() or sha256(tool_path) != expected:
    ...
    if not rom_two_path.exists():
        failures.append("ROM-closed two-residual checkpoint is missing")
    else:
        rom_two = json.loads(rom_two_path.read_text(encoding="utf-8"))
        if (
            rom_two.get("schema_version") != 1
            or rom_two.get("parent_board_sha256") != "7cff68dcadbfb275a343a76cd30c4f18404a3aed771b4181873df704596ce041"
            or rom_two.get("board_sha256") != "acc3650acc4e4f4a90e585e16b638b0e40dd5ae78f123712b706ec7fb9bbb904"
            or rom_two.get("uncapped_unconnected") != 2
            or rom_two.get("electrical_blockers") != 0
            or (rom_two.get("track_dangling"), rom_two.get("via_dangling")) != (0, 0)
            or set(rom_two.get("closed_targets", [])) != {"ROM_CS_EXP18", "ROM_SEL", "BA14"}
            or set(rom_two.get("residuals", [])) != {"BA2", "BA15"}
        ):
            failures.append("ROM-closed two-residual checkpoint is malformed")
        for relative, expected in rom_two.get("tool_sha256", {}).items():
            tool_path = ROOT / relative
            if not tool_path.is_file() or sha256(tool_path) != expected:
