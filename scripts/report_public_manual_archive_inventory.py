#!/usr/bin/env python3
"""Generate a classified inventory of public Juku manual/drawing archives."""
...
REPORT = ROOT / "docs" / "public-manual-archive-inventory.md"
...
REQUIRED_LOCAL = [
    ...
    "ref/schematics/es101_klaviatuur.pdf",
    "ref/schematics/es101_nimekiri_komponendid.pdf",
    "ref/schematics/juku_es101_processor_module.pdf",
    "ref/baltijets-tech-docs/001 Technical specifications and testing.pdf",
    "ref/baltijets-tech-docs/002 Schematics and components.pdf",
    "ref/baltijets-tech-docs/003 Adjustment instructions.pdf",
    "ref/baltijets-tech-docs/007 ROM and ROM programming.pdf",
    "ref/baltijets-tech-docs/009 FDDs.pdf",
    "ref/baltijets-tech-docs/012 Power supply.pdf",
    "ref/baltijets-tech-docs/013 Keyboard.pdf",
    "ref/baltijets-tech-docs/015 Floppy disk.pdf",
]
...
def emit_inventory(title: str, rows: list[tuple[str, str, str, str, str, str]]) -> list[str]:
    ...
    lines.extend(
        table_row([f"`{name}`", size, date, disposition, f"`{local}`" if local != "-" else "-", role])
        for name, size, date, disposition, local, role in rows
    )
    lines.append("")
    return lines
...
def main() -> int:
    missing = [path for path in REQUIRED_LOCAL if not exists(path)]
    status = "PUBLIC MANUAL ARCHIVE CLASSIFIED" if not missing else "PUBLIC MANUAL ARCHIVE INCOMPLETE"
    ...
    lines = [
        ...
        "| Check | Result |",
        "| --- | --- |",
        table_row(["Board-critical schematic/component PDFs exist locally", "PASS" if not missing else "FAIL"]),
        table_row(["Arti root manual/drawing listing classified", "PASS"]),
        table_row(["Elektroonikamuuseum `e5101-joonised/` listing classified", "PASS"]),
        table_row(["Elektroonikamuuseum `kirjandus/` listing classified", "PASS"]),
        "",
    ]
    lines.extend(emit_inventory("Arti Root Manual/Drawing Listing", ARTI_ROOT_ROWS))
    lines.extend(emit_inventory("Elektroonikamuuseum `e5101-joonised/` Listing", MUSEUM_DRAWING_ROWS))
    lines.extend(emit_inventory("Elektroonikamuuseum `kirjandus/` Listing", MUSEUM_LITERATURE_ROWS))
    lines.extend(
        [
            "## Disposition",
            "",
