...
def main() -> int:
    ...
    ready_guard = ROOT / "sync/d2_ready_path_check.sh"
    ready_tb = ROOT / "hdl/sim/d2_ready_path_tb.v"
    ready_guard_ok = (
        ready_guard.exists()
        and ready_tb.exists()
        and "D2-READY-PATH: PASS raw 00->READY 0, raw F/disabled->READY 1"
        in ready_tb.read_text()
        and "open collector" in (ROOT / "hdl/devices.v").read_text()
    )
    if not ready_guard_ok:
        raise SystemExit("D2 open-collector READY path guard missing or changed")
    lines = [
        ...
        "  Complete cycle-by-cycle WAIT duration still depends on the measured",
        "  `H` edge contact and surrounding clock/control timing.",
