...
| P0 | D94 .092 live steering capture | owner continuity on 2026-07-21 closes D94 D5-D7/pins6,7,9 and D104.10 as NC, completing the former static-output batch. Optionally scope D101.7, D94.1, /RE, and /WE on port 1F to observe the table's register-3 steering behavior | `docs/d94-reconstruction-constraints.md`; `docs/photo-registration.md`; exact two-sided local-fit rows in `ref/photos/juku-pcb-2/endpoints.csv` | uses the recovered PROM equations to target D0's hidden branch and validates the read/write steering dynamically before an FDC hardware release |
