...
| P0 | D94 .092 remaining outputs | the 2026-07-19 session closes A3 as D105.3 qualified /WR, separates D104.7 and raw D5.27, maps R87/R88/R89 to D94.4/.3/.2, maps R8 2 kΩ to the pull-up-only D94.1 branch, and retracts D94.5-D93.1. Trace only D94 D5-D7 far destinations and D104.10; optionally scope D101.7, D94.1, /RE, and /WE on port 1F to observe the table's register-3 steering behavior | `docs/d94-reconstruction-constraints.md`; `docs/photo-registration.md`; exact two-sided local-fit rows in `ref/photos/juku-pcb-2/endpoints.csv` | uses the recovered PROM equations to target D0's hidden branch, replaces the retired same-as-D8 BA mapping with measured row semantics, and resolves the read/write control path before an FDC hardware release |
...
| P0 | FDC support signal dispositions | pin-level continuity or an explicit redesign/DNP decision for the still-open D28, D97-D99, D101, and D102 pins. Preserve the source-closed D95 clock mux, D106 recovery counter, complete D96 read-clock toggle, D106.7-D28.9-D28.8-D96.3-D96.5-D93.26 chain, and D97/D102/D101 write-precomp chain. D101.1/.3/.5/.6, D97.13, and D102.4 remain the specific precomp-area boundaries; D96/D106 need no continuity probe, though their waveforms still need bring-up validation | `docs/fdc-hardware-handoff.md`; `ref/schematics/fdc-clock-mux-map.md`; `ref/schematics/fdc-recovery-counter-map.md`; `ref/schematics/fdc-read-clock-toggle-map.md`; `ref/schematics/fdc-write-precomp-map.md`; `PLAN.md` P0 connectivity gate | completes only the genuinely open support-circuit context without re-probing source-closed timing paths |
...
- Generated bring-up verification nets: `67`
- `FDC`: `7` net(s)
- `logic`: `39` net(s)
