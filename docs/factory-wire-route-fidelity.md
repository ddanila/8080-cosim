...
The owner backside resolves the right-hand printed joints directly in
`200522685`: A14B `(1825,2827)` maps to `(224.478,193.144)` mm on
the D35.12-side `PHI2` island, while A7B `(1757,2854)` maps to
`(227.668,194.422)` mm on the D35.10-side `PHI1` island. Their
3.437 mm separation and distinct printed numbers share the exact raw
image and D41.1-rebased solder-grid frame. D41.1 `(1451,1620)` is
58.911 mm from A14B, proving the former 0.784 mm collision was a
displaced D40-local projection. At the D1 end, the
...
214.476 mm and 226.473 mm respectively; both fit within the approximate
23/24 cm table entries, so endpoint geometry is adopted while exact
