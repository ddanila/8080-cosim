...
|---|---|---|---|---|
| Sorgelig / MiST `wd1793.sv` | `https://github.com/sorgelig/ZX_Spectrum-128K_MIST/blob/master/wd1793.sv` | Mature WD1793/WD1772/WD1773 replica with write capability; supports 10 x 512-byte sector geometry via `size_code`; expects RAM/SD image buffering rather than direct simulator file reads. | File header is GPL v2 or later. Rechecked 2026-07-09. | Good technical candidate if GPL HDL is acceptable in-tree or isolated under a clear vendor boundary. Needs a Juku adapter for `cs_n/rd_n/wr_n/A/D`, side/motor/ready, and raw image preload. Do not vendor while the local shim covers the guarded Juku boot path. |
| MiSTer SAM Coupe `wd1793.sv` | `https://github.com/MiSTer-devel/SAM-Coupe_MiSTer/blob/master/rtl/wd1793.sv` | Same Sorgelig-lineage style: full-featured compared with our boot shim, but not a drop-in for the current testbench. The GitHub file view exposes the SD/image-buffer interface rather than a direct raw-file simulator API. | Same lineage/license risk as above. Rechecked 2026-07-09. | Same as above. Prefer a single vendored source of this lineage if chosen. |
...
| Emu80v4 `Fdc1793` | `https://github.com/vpyk/emu80v4/blob/master/src/Fdc1793.cpp` | Software emulator model for КР580ВГ93/FDC1793 used by multiple Soviet-machine targets. It covers Type-I step family, sector read/write, read-address, write-track, DRQ/IRQ, index pulse status, write protect, and raw sector-image geometry. No Juku-specific driver/config was found in the 2026-07-08 recheck. | Repository is GPL-3.0-or-later. | Do not copy code into the local shim. Keep as a behavioral checklist if decoded FDC I/O is reached and the current boot/media shim proves insufficient. Details recorded in `docs/emu80v4-survey.md`. |
| Local WD1772 transistor/PLA reference | `ref/wd1772-vg93/wd1772.pdf`, `ref/wd1772-vg93/wd1772pla.txt` | User-provided reverse-engineering material for the WD1772/FD1773/VG93 lineage. The PDF is a searchable KiCad transistor/gate schematic; the text file is a 120-row PLA/PLM table with 19 input and 19 output columns. The source note says VG93 is effectively a direct FD1773/WD1772 copy, but `SHT_xxx` terms need normalization before equation comparison. | Vendored as reference material, not HDL-derived source. | Keep hashes and usage boundary in `docs/wd1772-vg93-reference.md`; use only as a deep signal/state-machine cross-check if a full controller model is required. |
...
1. Keep the local `fdc_1793` shim bounded to the guarded Juku boot/media path:
   restore/seek/step/read-sector/status/data/DRQ/INTRQ plus raw
   `media/disks/JUKU1.CPM` streaming.
2. Do not vendor a full upstream FDC core solely for the current plan state:
   reset-driven `juku_top` already reaches the EKDOS `A>` prompt and disk-side
   `JBASIC` reaches `READY` through the local shim.
3. If controller behavior beyond the current read path becomes a blocker
   (write-track, real flux/MFM timing, multi-sector edge cases, or external
   floppy-emulator interoperability), decide whether to accept GPL HDL and
   vendor/adapt Sorgelig-lineage `wd1793.sv`, using the Emu80v4 software model
   and local WD1772/PLA reverse-engineering files as additional behavior
   checklists.
