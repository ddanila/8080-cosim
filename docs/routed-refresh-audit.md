...
electrical blockers. Ninety-four bounded proposals across P5V, STSTB, GND,
...
D92_NOACC, D100_CONTROL_SHEET1_BOUNDARY, D33_CLK_RC, RES_RC, and FDC_READY then
each pass an independent uncapped check, reducing the exact count to 583;
those routes are promoted. The last ninety are selected transactionally
by `close_unconnected_gaps_uncapped.py`, which
