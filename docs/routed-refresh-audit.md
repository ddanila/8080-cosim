...
electrical blockers. One hundred eighteen bounded proposals across P5V,
...
OSC_PRE, SEP_D28_CLK, D97_C1_C16, D102_RC2_C20_R108, FDC_INTRQ, and
PRECOMP_CASCADE_2 then each pass an independent uncapped check, reducing the
exact count to 559; those routes are promoted. The last one hundred fourteen
are selected transactionally by `close_unconnected_gaps_uncapped.py`, which
