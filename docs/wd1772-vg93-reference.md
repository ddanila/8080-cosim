...
| `ref/wd1772-vg93/wd1772pla.txt` | `687a62103ae5a89a3daf4c1decb8968d730802522fa142e458031545b7a34b10` | ASCII PLA/PLM table. It has 120 product rows, 19 input columns, and 19 output columns; one row contains `9` markers and needs interpretation before machine use. |

The vendored hashes are guarded by `sync/reference_artifact_check.sh`, which is
part of the CI status-audit job.
`docs/wd1772-pla-inspection.md` is generated by
`scripts/report_wd1772_pla_inspection.py` and records the table shape,
duplicates, and ambiguous `9` markers.
`scripts/export_wd1772_pla.py` additionally emits normalized JSON/CSV copies
under `ref/wd1772-vg93/` for future equation-level work.
