...
not a replacement memory decoder and does not reinstate the diagnostic oracle.
...
PIC/keyboard boundary. In that historical experiment, restoring the explicit
oracle returned the guard to `PASS` at 25,744 resumed machine cycles.
The subsequent provisional adoption instead runs `juku_top` from physical
`U_DECODE` with only the documented `~D0`/`~D3` output correction; the
oracle remains instantiated here solely for the comparison above.
