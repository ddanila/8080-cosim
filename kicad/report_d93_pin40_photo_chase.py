#!/usr/bin/env python3
"""Guard D93.40 registration and rank safe +12 V continuity anchors."""
...
def main() -> int:
    ...
    boundary_nodes = {tuple(node) for node in spec["nets"]["D93_VDD12_BOUNDARY"]["nodes"]}
    observations = {}
    ...
    checks = [
        ...
        ("Source model keeps D93.40 off P12V pending continuity", ("D93", "40") not in p12_nodes),
        ("Source model exposes exactly one D93.40 boundary", boundary_nodes == {("D93", "40")}),
        ("Source PCB assigns D93.40 to the boundary", target.GetNetname() == "D93_VDD12_BOUNDARY"),
    ...
    status = "PAD GUARDED / NEAREST P12V ANCHORS RANKED / CONTINUITY REQUIRED" if ok else "D93 PIN40 CHASE FAILED"
    ...
    lines = [
        ...
        f"- Source-PCB D93.40 pad centre: `({tx:.3f}, {ty:.3f})` mm on `D93_VDD12_BOUNDARY`.",
