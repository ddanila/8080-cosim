...
def scenario_command(name: str, serial: str,
                     console: str | None = None) -> tuple[list[str], Path, str]:
    server = str(ROOT / "tools" / "janet_disk_server.py")
    if name == "cpm22":
        return ([
            sys.executable, "-u", server, serial,
            str(CPMISH / "juku-net-v2-system.bin"),
            str(CPMISH / "juku-net-mode2.img"),
        "--disk-baud", "9600", "--disk-protocol", "2",
        ], ROOT / "roms" / "ekta37.bin", "TN0201")
    network_rom = name == "cpm31_netrom"
    command = [
        sys.executable, "-u", server, serial,
        str(CPM3 / ("out/cpm-plus-juku-network-rom-extended-native-system.bin"
            if network_rom else "prebuilt/cpm-plus-juku-system.bin")),
        str(CPM3 / "out" / "cpm-plus-juku-full.img"),
        "--fast-stage1",
        str(CPM3 / ("out/cpm-plus-juku-network-rom-extended-native-fastboot-v16.bin"
            if network_rom else "prebuilt/cpm-plus-juku-fastboot-v15.bin")),
        "--disk-baud", "19200", "--disk-protocol", "3",
        "--disk-read-ahead-records", "8",
    ]
    if network_rom:
        command.append("--network-rom")
    else:
        command.extend(("--compact-stock-execute", "--fast-low-latency-guards"))
    if console:
        command.extend(("--console-pty", console))
    rom = ROOT / "spinoffs" / "jukuravi" / "network-rom" / \
        "juku-network-rom-abi1.2-c6.bin" if network_rom else \
...
def drive_remote_console(fd: int, name: str, started: float,
    ...
    pending = bytearray()
    commands = (
        ((b"DIR\r", None), (b"VER\r", None))
        if name == "cpm31_stock" else (
            (b"VER\r", None),
...
def drive_stock_keyboard(fd: int, started: float,
    ...
    while time.monotonic() < deadline:
        if any("Janet boot complete" in line for _, line in logs):
            time.sleep(2)
...
def run_scenario(name: str, output: Path, trace: Path) -> \
    ...
    command_stub, rom, keys = scenario_command(name, "SERIAL")
    require_files([rom, *(Path(item) for item in command_stub if "/" in item and
                              not item.endswith("janet_disk_server.py"))])
    ...
    trace_thread = threading.Thread(
        ...
        args=(cosim.stderr, started, [], ready, serial), daemon=True,
    )
    trace_thread.start()
    if not ready.wait(10):
        cosim.terminate()
        raise RuntimeError(f"{name}: simulator did not expose its USART")
    if name == "cpm31_netrom":
        console_master, console_slave = pty.openpty()
        tty.setraw(console_slave)
    command, _, _ = scenario_command(
        name, serial[0], os.ttyname(console_slave) if console_slave is not None else None)
    ...
    try:
        if name == "cpm22":
            ...
            time.sleep(2)
            logs.append((time.monotonic() - started,
                         "Stopping Janet server after completed capture"))
            server.terminate()
            server.wait(timeout=10)
            cosim.terminate()
        ...
        for fd in (console_master, console_slave):
            if fd is not None:
                os.close(fd)
...
def render(name: str, capture: Path, logs: list[tuple[float, str]],
    ...
    title = {
        ...
        "cpm31_stock": "Stock ROM + fast CP/M Plus 3.1",
    ...
    for position, current in enumerate(timeline):
        ...
        draw.text((left_width + 10, 14),
                  f"Host / Janet server   elapsed {current:6.2f}s",
...
def main() -> int:
    ...
    parser.add_argument("--scenario",
                        choices=("cpm22", "cpm31_stock", "cpm31_netrom", "all"),
                        default="all")
    ...
    build_trace(trace)
    names = ("cpm22", "cpm31_stock", "cpm31_netrom") \
    ...
    for name in names:
        ...
        gif = output / {
            ...
            "cpm31_stock": "stock-rom-fast-cpm31.gif",
