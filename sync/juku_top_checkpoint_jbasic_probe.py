...
def run_hdl(tmp: Path, ram_bin: Path, state: dict[str, str]) -> tuple[subprocess.CompletedProcess[str], bool, bytes]:
    ...
    args = [
        ...
        f"+timecap={os.environ.get('JUKU_TOP_CHECKPOINT_JBASIC_TIMECAP', '3200000000')}",
        ...
        f"+stopfdc_data_reads={os.environ.get('JUKU_TOP_CHECKPOINT_JBASIC_STOP_DATA_READS', '512')}",
        "+stopjbasicready=1",
    ]
    ...
    try:
        with out_path.open("w") as stdout, err_path.open("w") as stderr:
            proc = subprocess.run(
                ...
                stdout=stdout,
                stderr=stderr,
                timeout=int(os.environ.get("JUKU_TOP_CHECKPOINT_JBASIC_TIMEOUT", "360")),
                check=False,
            )
