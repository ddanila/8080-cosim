...
- `scripts/report_d94_reconstruction_constraints.py` — generated D94 `.092`
  ...
  across the repository artifact scan; the generated 32-row address-space table
  leaves 256 content bits unresolved before any burnable replacement is attempted.
...
- `sync/serial_check.sh` / `scripts/report_serial_handoff.py` — generated D11
  USART/X3 serial guard: verifies the host bus, D57 baud clock,
  line-driver/receiver chain, X3 connector nets, and a minimal 8251-style
  mode/command/status plus 8N1 digital loopback behavior slice. External X3
  loopback and full protocol modes remain bring-up boundaries.
