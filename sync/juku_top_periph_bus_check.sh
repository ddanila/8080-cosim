...
| Timed Type-I physical-head/update, HLT-gated verify with immediate valid-ID mismatch, and exact 15-idle-index HLD release through decoded ports \`0x1C..0x1F\` | $status |
| Forced TR00 proves live TRACK 0 status plus outward Restore stepping and completion when active-low TR00 asserts | $status |
| Forced-low HLT holds Type-III BUSY/DRQ-low and its rising edge starts media access | $status |
...
| Type-II multi-write re-arms the 22-byte preload interval, persists sectors 9/10, and ends at sector 11 with RNF | $status |
