...
module prom_fallback_tb;
  ...
  reg d6_probe_enable_n;
  reg [7:0] d6_probe_address;
  wire d6_probe_rom_n, d6_probe_ram_n, d6_probe_rev, d6_probe_roe_n;
  integer i;
  integer errors = 0;

  decode_prom U_D6(.a(a8), .v_en_n(1'b0), .rom_n(rom_n), .ram_n(ram_n), .rev(rev), .roe_n(roe_n));
  decode_prom U_D6_OC_PROBE(.a(d6_probe_address), .v_en_n(d6_probe_enable_n),
      .rom_n(d6_probe_rom_n), .ram_n(d6_probe_ram_n),
      .rev(d6_probe_rev), .roe_n(d6_probe_roe_n));
  ...
  initial begin
    ...
    d6_probe_enable_n = 1'b1;
    d6_probe_address = 8'h00;
    ...
    if ({d6_probe_roe_n, d6_probe_rev, d6_probe_ram_n, d6_probe_rom_n} !== 4'bzzzz) begin
      $display("PROM-FALLBACK: D6 disabled outputs did not release: %b",
