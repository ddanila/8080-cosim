...
| `JRV-HW` | Jukuravi / network ROM | CS00015 EKTA4401/JUKURAVI SERVICE ROM FITTED IN HOME LAB; CORRECTED CP/M PLUS V3 DISK PATH PHYSICALLY PASSES `A>`, `DIR`, AND `DIAG`, BUT STOCK-`TN` FINAL V15 COMPLETION HANDOFF REMAINS OPEN; NETWORK-FIRST ABI, POST, KEYLESS 19,200 BOOT, AND REAL CP/M PLUS HANDOFF PASS IN COSIM, WITH D15/D16 STILL MARKED DO-NOT-PROGRAM; CS00014 STOCK-ROM EXHIBIT; CS00000 USART FAULT SUSPECTED; OLD D55 PREDICATE INVALIDATED | migrate serial/memory-mode, keyboard, console/font, and NetDisk services behind the resident ABI; relink and measure TPA; complete recovery simulation before releasing programmer artifacts; preserve the frozen stock/RAM-BIOS baseline and CS00014 exhibition state; isolate CS00000 serial failure before any D11 diagnosis | `docs/machine-deployment-status.md`; `docs/cs00015-service-record.md`; `docs/serial-handoff.md`; `docs/juku-serial-19200-investigation.md`; [`spinoffs/jukuravi/network-rom/README.md`](spinoffs/jukuravi/network-rom/README.md); [network-first ROM plan](https://github.com/ddanila/cpm-plus-juku/blob/master/docs/network-first-rom-plan.md) |
