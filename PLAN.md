...
1. **Complete FDC-era functional wiring.** D93's drive-interface pins plus
   ...
   table leaves two minimal sufficient families: qualified enable
   (`/OE=FDC_CS_N`, `T=IORD`) or the D23-D25 same-board precedent
   (`/OE=GND`, `T=D93_RE_N`). Both keep `T=1` for writes, set `T=0` only for
   selected reads, and prevent D100 from driving CPU DB when unselected. Both
   are exhaustively guarded, as is D25's newly bidirectional model under its
