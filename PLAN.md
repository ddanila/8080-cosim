...
| `JRV-HW` | Jukuravi / network ROM | CS00015 EKTA4401/JUKURAVI SERVICE ROM FITTED IN HOME LAB; CORRECTED CP/M PLUS V3 DISK PATH PHYSICALLY PASSES `A>`, `DIR`, AND `DIAG`, BUT STOCK-`TN` FINAL V15 COMPLETION HANDOFF REMAINS OPEN; NETWORK-FIRST 16 KiB ROM ARCHITECTURE PLANNED; CS00014 STOCK-ROM EXHIBIT; CS00000 USART FAULT SUSPECTED; OLD D55 PREDICATE INVALIDATED | close the stock-`TN` completion signal; finish the RAM compact-console/cursor oracle; then implement the subordinate network-first ROM plan without displacing the frozen stock/RAM-BIOS baseline; preserve CS00014's exhibition state; isolate CS00000 serial failure before any D11 diagnosis | `docs/machine-deployment-status.md`; `docs/cs00015-service-record.md`; `docs/serial-handoff.md`; `docs/juku-serial-19200-investigation.md`; [network-first ROM plan](https://github.com/ddanila/cpm-plus-juku/blob/master/docs/network-first-rom-plan.md) |
