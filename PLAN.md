...
| `JRV-HW` | Jukuravi / network ROM | CS00015 EKTA4401/JUKURAVI SERVICE ROM FITTED IN HOME LAB; CORRECTED CP/M PLUS V3 DISK PATH PHYSICALLY PASSES `A>`, `DIR`, `DIAG`, AND `WBOOT` AFTER MANUAL 19,200 RESUME, BUT STOCK-`TN` FINAL V15 COMPLETION HANDOFF REMAINS OPEN; NETWORK-FIRST C2 ABI, POST, KEYLESS 19,200 BOOT, REAL CP/M PLUS HANDOFF, CORRECTED SOURCE-FONT FRAMEBUFFER, AND STRUCTURAL HDL GATES PASS; C2 IS READY FOR CONTROLLED BURN BUT UNPROMOTED; CS00014 STOCK-ROM EXHIBIT; CS00000 USART FAULT SUSPECTED; OLD D55 PREDICATE INVALIDATED | burn the exact C2 D15/D16 pair and complete the auditable CS00015 cold/warm boot, display/cursor, keyboard, disk write, host-loss/reconnect, and repeated-timing matrix; preserve the stock/RAM-BIOS baseline and CS00014 exhibition state; keep the stock-`TN` wrapper defect separate; isolate CS00000 serial failure before any D11 diagnosis | `docs/machine-deployment-status.md`; `docs/cs00015-service-record.md`; `docs/serial-handoff.md`; `docs/juku-serial-19200-investigation.md`; [`spinoffs/jukuravi/network-rom/README.md`](spinoffs/jukuravi/network-rom/README.md); [network-first ROM plan](https://github.com/ddanila/cpm-plus-juku/blob/master/docs/network-first-rom-plan.md) |
...
Until new `MAIN-P0` measurements are accepted, physical-fidelity edits to the
board model, KiCad sources, structural connectivity, promoted route, or
fabrication package are paused; simulation and test work must preserve that
evidence boundary.
...
1. **Physical D6 `.038` firmware — CONFIRMED and directly adopted
   ...
   oracle across the full suite — `boot_check`, `cosim_check`
   (`BTRACE-END`), EKDOS `A>`, disk-BASIC `READY`, jmon33 Monitor, BASIC-cart,
...
1. **Footprint review items:** exact HRO TYPE-C-31-M-17/C283540 USB-C
   ...
   `spinoffs/minimal-vga/docs/rev-a-ptc-candidate.md`. Exact Littelfuse
   P4KE6.8A-B/C1666224 TVS compatibility is now checksum-, polarity-,
   topology-, geometry-, and routed-clearance-guarded in
   `spinoffs/minimal-vga/docs/rev-a-tvs-candidate.md`. Automation still cannot
...
3. **Disposition full-board LVS explicitly — STAGE 6 DONE, remainder open.**
   ...
   they touch (28 mapped refs / 37 partitions / 5 NC pads). Stage 3 maps every
...
single-driver correction atomically. The promoted route passes its connectivity
and DRC guards; exact topology is checked by
...
- [x] Deep value-level cosim guard reaches `BTRACE-END` and fails on any
  type/address/data divergence.
- [x] Independent C 8080 conformance guard covers ALU/flags, DAA, rotates,
  stack bus order, I/O, EI, and undocumented aliases.
- [x] Generated instruction-level C/vm80a differential covers every opcode and
  architectural flag combination, including memory and port effects.
- [x] Generated C/HDL FDC differential covers the declared command/event
  scenarios, writable media, DRQ deadlines, Read Address, and track streams
  without claiming physical timing.
