...
| PCB package | The tracked routed artifact (240 footprints) is DRC-clean within its modeled scope, but predates accepted D2/D94, reset/USART, and harness endpoint changes. A separate source-complete refresh candidate (296 footprints, exact pad/net parity) is converging: 64 unconnected items on 50 nets remain, with zero shorts, clearance, crossing, or hole findings (`docs/routed-refresh-audit.md`) | Close the remaining 64 connections plus the dangling `OSC` track and one `GND` edge-clearance finding; adoption additionally waits for the functional netlist to stop changing |
...
2. **Continue routing convergence on the refresh candidate.** The
   ...
   distance bands to 225 mm and corridor sweeps to 120 mm at 64 unconnected
   items on 50 nets. Next: partition or hand-route the residual nets, and
   clear the dangling `OSC` track and the `GND` edge-clearance finding.
   Track work in `docs/routed-refresh-audit.md`. Copper produced now survives
   later netlist changes through the per-net quarantine mechanism, but do not
   replace `kicad/juku_routed.kicad_pcb` until the P0 netlist is frozen and
   endpoint parity plus zero electrical DRC findings are both proved.
