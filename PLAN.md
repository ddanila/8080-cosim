...
| `JRV-HW` | Jukuravi harness | PHYSICAL T31/T32 VALIDATED; D1 FAULT REPAIRED; D55 OPEN | execute the controlled D55 before/substitute/after T15/T16 matrix; keep optional Nano reset/liveness wiring measurement-gated | `spinoffs/jukuravi/D55-REPLACEMENT.md`; `docs/cs00015-service-record.md` |
...
1. **Physical D6 `.038` firmware — CONFIRMED and directly adopted
   ...
   oracle across the full suite — `boot_check`, the 130,000-event `cosim_check`
...
- [x] Deep value-level cosim guard reaches `BTRACE-END` across the default
  130,000 typed CPU-bus events and fails on any type/address/data divergence.
- [x] Independent C 8080 conformance guard exhaustively checks 4.48 million
  ALU/flag, DAA, rotate, stack bus-order, I/O, EI, and undocumented-alias
  assertions; the unified bus guard found and fixed high-byte-first PUSH order.
- [x] Generated instruction-level C/vm80a differential matches 8,192 isolated
  cases: all 256 opcode bytes crossed with all 32 S/Z/AC/P/C combinations,
  rotating boundary-value registers/operands, final memory effects, and port I/O.
- [x] Generated C/HDL FDC differential matches 12 deterministic command/event
  scenarios across 50,845 state transitions, including isolated writable media,
  exact DRQ deadlines, Read Address, and full track streams, without claiming
  physical timing.
