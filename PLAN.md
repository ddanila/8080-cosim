...
1. **Complete FDC-era functional wiring.** D93's drive-interface pins plus
   ...
   controller-side asks are the separately guarded DRQ/INTRQ assumptions and shared
   ...
   profile, and the actual CPU↔D93 DAL transform must be recovered from sheet 3
   or continuity rather than reassigned to another convenient buffer. The
...
5. **Disposition all remaining source-risk nets and omitted endpoints.**
   58 source-risk nets and 2 official FDC devices with untraced functional
