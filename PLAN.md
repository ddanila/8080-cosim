...
WREQ_N, PIT_BAUD, ROE, STSTB_D38, CTR_LD, RAM_OUT_EN, MA2, and MA1, each
independently reduce the uncapped graph, reaching 27 opens; all six hundred
fifty are promoted. The final six hundred forty-six are
...
its first `MA2` and `MA1` attempts pass the same complete KiCad DRC guard.
