...
1. **Complete FDC-era functional wiring.** D93's drive-interface pins plus
   ...
   table admits the minimal functional candidate `/OE=FDC_CS_N`, `T=IORD`:
   selected writes have `T=1` (CPU DB -> inverted DAL), selected reads have
   `T=0` (DAL -> inverted CPU DB), and deselection releases both buses. This is
   a behaviorally forced candidate, not copper evidence: pin 9 visibly ends at
   an isolated component-side landing and pin 11 is cable-obscured. With a
