...
| PCB package | The tracked routed artifact (240 footprints) is DRC-clean within its modeled scope, but predates accepted D2/D94, reset/USART, and harness endpoint changes. A separate source-complete refresh candidate (296 footprints, exact pad/net parity) is converging: 30 unconnected items on 29 nets remain as of 2026-07-14, with zero shorts, clearance, crossing, hole, or edge findings (`docs/routed-refresh-audit.md`) | Close the remaining connections plus the dangling end of the still-open `OSC` net; adoption additionally waits for the functional netlist to stop changing |
...
2. **Continue routing convergence on the refresh candidate.** The
   ...
   refinement to 0.10 mm; shape-aware pad, drilled-hole, and Edge.Cuts
   keep-outs then closed P5V, BA13, MEMR, P12V, and RESET, leaving 30
   unconnected items on 29 nets as of 2026-07-14. Next: partition
   or hand-route the residual nets and thereby clear the dangling `OSC` end.
   Track work
