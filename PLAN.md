...
| Connectivity | `sync/check.sh` reports 100 mapped instances and 242 matched nets; the complete D59/Z1/C73/R31/R32 oscillator loop and S4 SPDT interrupt selector are source-modeled and LVS-visible | Unmapped footprints, omitted pins, behavioral correctness, the analog oscillator waveform, and historical correctness of assumed nets |
...
The source PCB now passes all `2021/2021` PCB-scoped board-JSON endpoints.
