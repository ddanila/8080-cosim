...
| --- | --- | --- |
| Digital twin | `cosim` and `juku_top` boot ekta37; framebuffer and keyboard guards pass; uninterrupted HDL reaches EKDOS `A>` and disk BASIC `READY`; the recovered ROMBIOS `0xA0/0xA2` 512-byte write-sector path is implemented in both models and byte-for-byte readback-tested on an explicitly writable disk copy; the C and HDL FDC models share guarded restore/seek/step direction and track-register-update semantics, Type-II multi-record continuation through end-of-track RNF, datasheet completion/status-acknowledgement plus silent-D0/immediate-D8 INTRQ behavior, a CRC-checked six-byte Read Address command, and a 6,250-byte one-revolution MFM Read Track reconstruction with all ten ID/data fields and CRCs; Monitor 3.3 reaches its cursor and selected commands; the cosim-referenced deep guard reaches `CTRACE-END` across 130,000 reads; runnable selection comes from the validated physical D6 table under the explicitly provisional `~D0`/`~D3` fit, while the old functional decoder is retained only as a diagnostic comparison; the official D1-D5-DB-ROM topology and period КР580ВК38/ВА87 diagrams exclude a hidden D5 inversion, exact static/runtime guards prove the preserved CMA/NOP firmware split, and all four physical ВА87 models now exhaustively guard all 256 values in both inverting directions; two opt-in top-level builds pass restore/seek/media/read-track/write through physical D100/DAL under both safe `/OE` families with actual D93 `/RE` direction, including D94's suppressed-read branch | Confirm or remove the D6 per-output fit with the corrected-reader re-read or operating-level probe across the separate D6.12/D8 ROM-select and D6.9/D13/D37/D58 timing paths, and close its physical A7 driver; identify the exact fitted D15/D16 polarity profile and continuity-select D100's chip-select-qualified or always-enabled `/OE` family while proving `T=D93_RE_N` (or an equivalent that honors suppressed reads); exact physical shared-DRAM video-slot/DOUT timing, complete controller behavior beyond the guarded Type-I/II plus Read Address/Read Track subset (including writable formatting, lost-data deadlines, and transition/index Force modes), cartridge BASIC loading, and analog behavior |
