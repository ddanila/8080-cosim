...
| Connectivity | `sync/check.sh` reports 101 mapped instances and 256 matched nets; physical D2/D6 PROM tables, the measured D2/D30/D105/D13 READY/DBIN handoff, D41 timing rails 8/17, and the X1.107C `-WREQ` endpoint are source-modeled and LVS-visible | Routed-snapshot parity, omitted remote endpoints, behavioral correctness, analog waveforms, and historical correctness of assumed nets |
...
The source PCB now passes all `2230/2230` unique PCB-scoped board-JSON endpoints; the
