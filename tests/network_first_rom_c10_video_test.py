#!/usr/bin/env python3
"""Regress the physical C9 blank-video fault and the exact C10 correction."""
...
def run_fixture(trace: Path, image: bytes, case: Path) -> tuple[dict[str, str], bytes, list[int]]:
    rom = case / "rom.bin"
    ...
    return state, ram[0xD800:0xD800 + 9600], control_writes
...
def main() -> int:
    ...
    if c9_metadata["hardware_init"][0] != "ppi0-82-pc7-high" or \
            c10_metadata["hardware_init"][0] != "ppi0-82-pc7-high-then-low":
        fail("C9/C10 hardware-init metadata does not encode the discriminator")
    ...
    with tempfile.TemporaryDirectory(prefix="network-rom-c10-video.") as name:
        ...
        c9_case.mkdir()
        c10_case.mkdir()
        c9_state, c9_frame, c9_writes = run_fixture(trace, c9, c9_case)
        c10_state, c10_frame, c10_writes = run_fixture(trace, c10, c10_case)
    ...
    if not any(c9_frame) or c9_frame != c10_frame:
        fail("raw C9/C10 framebuffer evidence differs or is empty")
    if any(visible_pixels(c9_frame, 0x81)):
        fail("C9 POF-high negative control is not classified blank")
    if visible_pixels(c10_frame, 0x01) != c10_frame or not any(c10_frame):
